...
- **Test Contract Closure Rule** (see `docs/architecture/00-index/agent-guardrails.md#Test-Contract-Closure-Rule` and [AGENTS.md](../../../AGENTS.md)) — every bullet in every relevant spec's `## Test Contract` section must be in one of three states at handoff: passing test with verbatim name / skipped with reason / declared-out-of-scope in the slice work log. This skill surfaces silent omissions; don't suppress them.
