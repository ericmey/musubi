...
4. `gh pr create --draft --base main` with **first line of body = `Closes #<n>.`** (exact keyword; GitHub doesn't auto-link otherwise).
...
- **When a PR sits in-review while main advances, the PR branch goes stale.** Check `gh pr view <pr> --json mergeStateStatus` before claiming handoff green. If it returns `DIRTY`, CI is being suppressed; merge or rebase `main` into the branch first. "`gh pr checks <pr>` shows no results" + "`statusCheckRollup` is empty" both mean CI skipped, not CI setup missing.
...
- **"Make check passed locally" does NOT mean "what I pushed will pass CI."** Before claiming handoff, run `git status`, `git diff --stat`, and `git ls-files <your-owns-paths>/` to verify that your implementation code is actually tracked and staged. If your local tests pass but CI fails on ImportError/missing-module, it means you forgot to commit your implementation files. Always verify the commit graph contains your implementation code, not just your tests, before claiming green.
