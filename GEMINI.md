...
- **When a PR sits in-review while v2 advances, the PR branch goes stale.** Check `gh pr view <pr> --json mergeStateStatus` before claiming handoff green. If it returns `DIRTY`, CI is being suppressed; merge or rebase `v2` into the branch first. "`gh pr checks <pr>` shows no results" + "`statusCheckRollup` is empty" both mean CI skipped, not CI setup missing.
