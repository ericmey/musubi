...
def _make(content: str, namespace: str, from_presence: str, to_presence: str, **extra: object) -> Thought:
...
async def test_thought_read_broadcast_appends_to_read_by_only(plane: ThoughtsPlane, ns: str) -> None:
...
async def test_thought_read_batched_not_N_plus_1(plane: ThoughtsPlane, ns: str) -> None:
    ...
    updated_list = await plane.read_batch(namespace=ns, object_ids=[t1.object_id, t2.object_id], reader="b")
...
async def test_thought_history_semantic_match(plane: ThoughtsPlane, ns: str) -> None:
    ...
    history = await plane.history(namespace=ns, channel="default", query="fruit", limit=10)
...
async def test_thought_in_reply_to_chain_queries_correctly(plane: ThoughtsPlane, ns: str) -> None:
    ...
    replies = await plane.history(namespace=ns, channel="default", in_reply_to=t1.object_id, limit=10)
...
async def test_cross_tenant_thought_requires_multi_tenant_scope(plane: ThoughtsPlane, ns: str) -> None:
    ...
    with pytest.raises(ValueError, match="Cross-tenant thoughts require explicit multi-tenant scope"):
...
async def test_thought_embedding_deferred_under_load_does_not_block_send(plane: ThoughtsPlane, ns: str) -> None:
    ...
    records, _ = plane._client.scroll(
        ...
        scroll_filter=qmodels.Filter(
            must=[qmodels.FieldCondition(key="object_id", match=qmodels.MatchValue(value=t.object_id))]
        ),
        with_vectors=True,
        limit=1
    )
    assert not records[0].vector or "dense_bge_m3_v1" not in records[0].vector or records[0].vector["dense_bge_m3_v1"] == []
