...
def test_the_discovery_found_workflows_at_all():
...
def test_workflow_declares_permissions_at_some_scope(path: pathlib.Path):
    ...
    missing = [name for name, job in jobs.items() if (job or {}).get("permissions") is None]
    assert not missing, (
...
@pytest.mark.parametrize("path", FILES, ids=lambda p: p.name)
def test_no_workflow_grants_blanket_write(path: pathlib.Path):
    ...
    scopes += [(job or {}).get("permissions") for job in (doc.get("jobs") or {}).values()]
    for scope in scopes:
