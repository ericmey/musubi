...
async def test_mcp_adapter_surfaces_warnings():
    out = await _do_search(
        _WarningClient(), namespace="test/ns", query="q", limit=5, planes=["episodic"]
    )
...
async def test_livekit_fast_talker_surfaces_warnings():
    ft = FastTalker(client=_WarningClient(), namespace="test/ns", cache=ContextCache())
...
async def test_livekit_slow_thinker_surfaces_warnings():
    st = SlowThinker(client=_WarningClient(), namespace="test/ns", cache=ContextCache())
