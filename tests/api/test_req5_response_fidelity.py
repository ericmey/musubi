...
def test_replay_preserves_set_cookie(api_settings: Settings) -> None:
    ...
    assert replay.headers.get_list("set-cookie"), (
        "replay dropped Set-Cookie — a faithful replay must keep it"
    )
...
def test_non_json_response_is_idempotent_and_keeps_media(api_settings: Settings) -> None:
    ...
    assert replay.headers.get("content-type", "").startswith("text/csv"), (
        "replay lost the media type"
    )
...
def test_background_task_runs_once_and_not_on_replay(api_settings: Settings) -> None:
    ...
    async def h() -> Response:
        return JSONResponse(
            {"ok": 1}, background=BackgroundTask(lambda: runs.__setitem__("n", runs["n"] + 1))
        )
    ...
    assert runs["n"] == 1, (
        f"background task re-ran on replay (now {runs['n']}x) — replay must not repeat side effects"
    )
    assert replay.headers.get(REPLAY) == "true"
