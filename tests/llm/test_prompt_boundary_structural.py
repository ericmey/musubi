import pytest
import json
from musubi.llm.prompt_boundary import build_untrusted_data_messages, ChatMessage, JsonValue
...
def test_sec007_prompt_boundary_system_user_separation() -> None:
    ...
    payload: dict[str, JsonValue] = {
        "items": [
            {"id": malicious_id, "content": malicious_text}
        ]
    }
    ...
    assert len(parsed_payload["items"]) == 1  # Crucial: the attacker did not successfully inject `fake_id` as a second list item!
