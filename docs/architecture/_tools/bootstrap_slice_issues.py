...
One-shot script. Reads every ``docs/architecture/_slices/slice-*.md``, builds
...
Invocation:
    ...
    python3 docs/architecture/_tools/bootstrap_slice_issues.py              # dry run
    python3 docs/architecture/_tools/bootstrap_slice_issues.py --apply      # actually create
...
def render_body(s: Slice, all_slices: dict[str, Slice]) -> str:
    ...
    rel_path = f"docs/architecture/_slices/{s.path.name}"
    ...
    if s.specs:
        ...
        for spec in s.specs:
            lines.append(f"- [`docs/architecture/{spec}.md`](docs/architecture/{spec}.md)")
        lines.append("")

    if s.depends_on:
        lines.append("## Depends on")
        for dep in s.depends_on:
            lines.append(f"- `{dep}` (vault: `docs/architecture/_slices/{dep}.md`)")
        lines.append("")
        lines.append(
    ...
    lines.append(
        ...
        "([Closure Rule](docs/architecture/00-index/agent-guardrails.md#test-contract-closure-rule))."
    )
    lines.append("")
    ...
    lines.append(
        ...
        "[`agent-guardrails.md`](docs/architecture/00-index/agent-guardrails.md)."
    )
