...
Musubi is a **single-repo monorepo** per [[13-decisions/0015-monorepo-supersedes-multi-repo]] and [[13-decisions/0016-vault-in-monorepo]]. The 8-repo layout originally proposed in [[13-decisions/0011-canonical-api-and-adapters]] is retired; its interface discipline survives as import-lint rules. The Obsidian architecture vault lives in the same repo at `docs/architecture/`.

| Repo | Primary owner | Backup | Access | Contents |
|---|---|---|---|---|
| `github.com/ericmey/musubi` | Eric | — | private | Everything: Core, SDK, MCP/Obsidian/CLI adapters, contract tests, compose + Ansible under `deploy/`, *and* the Obsidian architecture vault under `docs/architecture/`. `v2` branch is the rebuild; `main` still holds the v1 POC until V2 cutover. |
