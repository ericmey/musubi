...
- **Adapter specs stay in Musubi even for non-Python components.** `docs/Musubi/07-interfaces/openclaw-adapter.md` is Musubi's spec of the contract; implementation in the external repo consumes that spec. Readers need to know to look at two places. Mitigation: each spec names the implementing repo in its opening paragraph.
