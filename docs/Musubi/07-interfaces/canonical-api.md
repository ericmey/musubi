...
**Replay on reconnect:** `Last-Event-ID: <ksuid>` triggers replay of every thought matching the subscription where `object_id > <ksuid>` (lexicographic, ascending) before entering live-tail mode. The server scrolls the thoughts plane filtered by `created_epoch >= <ksuid-timestamp>` at index level, **exhausts that filtered post-anchor slice**, and only then sorts by `object_id` and applies the cap. It must not stop early once `cap + 1` candidates are collected: scroll order is by point id, not by KSUID, so an early stop can return later events and skip the earliest ones. Cost: reconnect work scales with the number of thoughts matching the subscription in the post-anchor epoch window, not with the cap. A long disconnect in a busy namespace reads more rows. There is deliberately no scan cap, because one would reintroduce event loss.
...
and publishes the ordinary replay entry before releasing a successful response.
Failure of either pre-send step returns typed 503 and holds the process-local lease
fail-closed; the client recovers through receipt lookup. Ordinary POST replay without
that header retains its 24h TTL; the receipt ledger is independent and survives API
process restart. The durable mode requires `Idempotency-Key` and is deliberately
explicit so ordinary callers retain the existing key-reuse contract.
...
Reranker degradation is backward-compatible and additive. Every response with
a degraded rerank includes `reranker_failed`. Each degraded rerank with a known
cause additionally contributes one bounded cause code, so a merged multi-plane
response may carry more than one: `reranker_failed_timeout`,
