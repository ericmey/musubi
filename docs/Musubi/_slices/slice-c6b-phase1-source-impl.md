...
PR #473). No merge/deploy until independent review.
...
> in this successor; S5 flips R19 (PII-free bounded observability); S6 flips the three R20 behavior/process
> proofs plus G2b's exact cleanup SQL shape** — all EMPIRICALLY re-derived (not the §F paper matrix). Every other
...
23. `test_r20_rollback_refuses_nonterminal_maintenance_lifecycle_and_cleanup` — GREEN — S6 flip (durable maintenance/generation lifecycle, refuse/drop/handoff fences, bounded cleanup/backfill)
24. `test_r20_two_process_admission_drain_barrier_no_overlap` — GREEN — S6 flip (real-process admission LOCK_SH drained by rollback LOCK_EX, stable across 5x)
25. `test_r20_two_process_reconciler_drain_barrier_no_overlap` — GREEN — S6 flip (independent real-process reconciler LOCK_SH drain proof, stable across 5x)
...
- **S6 `rollback` — DONE (S6).** Durable maintenance control + stable shared/exclusive flock barrier,
  generation-fenced rollback/abort, terminal-age backfill, and bounded atomic cleanup landed. The three
  R20 stage guards and strict-xfail markers were removed; R20 admission drain, reconciler drain, and
  rollback/cleanup behavior plus G2b are GREEN. No `rollback` guard remains.
